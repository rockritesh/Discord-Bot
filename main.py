...
async def on_ready():
	while True:
		...
		await bot.change_presence(activity=discord.Activity(type=1,name="Selling Crowd script || Google Script"))
		await asyncio.sleep(5)
		
		await bot.change_presence(activity=discord.Activity(type=1,name="Developer By 𝚙𝚛𝚒𝚝𝚊𝚖™┇®[ᴅᴇᴠ]#3939"))
		await asyncio.sleep(5)
...
async def on_member_join(member):
    ...
    embed.set_footer(text="Developer By 𝚙𝚛𝚒𝚝𝚊𝚖™┇®[ᴅᴇᴠ]#3939")
...
async def start(ctx,game:str):
	...
	if BOT_OWNER_ROLE in [role.name for role in author.roles]:
		...
		embed.set_footer(text=f"Developer By 𝚙𝚛𝚒𝚝𝚊𝚖™┇®[ᴅᴇᴠ]#3939", \
...
async def end(ctx,game:str):
	...
	if BOT_OWNER_ROLE in [role.name for role in author.roles]:
		...
		embed.set_thumbnail(url=url)
		embed.set_image(url="https://cdn.discordapp.com/attachments/539066238870224903/606135147913543693/Tw_1-1-1.gif")
		embed.set_footer(text="Developer By 𝚙𝚛𝚒𝚝𝚊𝚖™┇®[ᴅᴇᴠ]#3939",icon_url="https://cdn.discordapp.com/attachments/731757131635621980/737706495726190664/Screenshot_20200723-104510_Google_Play_Store.jpg")
		embed.timestamp = datetime.datetime.utcnow()
...
async def invite(ctx):
	await ctx.send("**__@everyone Add Me to Your servers!__**\nhttps://discord.com/api/oauth2/authorize?client_id=749188746217259059&permissions=8&scope=bot")
...
async def purge(ctx,amount=1000):
    if 'RUNNER' in [i.name for i in ctx.message.author.roles]:
...
bot.run("NzQ5MTg4NzQ2MjE3MjU5MDU5.X0oWZA.qub2o-6mn1FEZNGNOo9x-6R6NFg")
